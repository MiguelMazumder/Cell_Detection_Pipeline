...
function [cell_counter, cell_loc_store, quadrant_count_vectorform] = cell_detect(base_image,quad_dim,current_image_name)
    ...
    quadrant_xboundries = linspace(0,size(base_image,1),quad_dim(1)+1);
    quadrant_yboundries = linspace(0,size(base_image,2),quad_dim(2)+1);
    cell_loc_store = [];
    for i = 1:numel(props_nucleus)
        ...
        if x > 1 && x < size(cell_stain, 2) && y > 1 && y < size(cell_stain, 1)
            ...
            if overlapRatio >= thresholdOverlap
                ...
                while x >= quadrant_xboundries(quadrant_xindex+1)
                    ...
                    if quadrant_xindex == quad_dim(1)
                        break
                    end
                end
                while y >= quadrant_yboundries(quadrant_yindex+1)
                    quadrant_yindex = quadrant_yindex + 1;
                    if quadrant_yindex == quad_dim(2)
                        break
                    end
                end
                quadrant_cellcount(quadrant_xindex,quadrant_yindex) = quadrant_cellcount(quadrant_xindex,quadrant_yindex) + 1;
            end
        end
    end
